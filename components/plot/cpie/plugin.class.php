...
class plugin_cpie extends plugin_base{

    public function init() {
        $this->fullname = get_string('cpie', 'block_configurable_reports');
        $this->form = true;
        $this->ordering = true;
        $this->reporttypes = array('courses', 'sql', 'users', 'timeline', 'categories', 'sqltde');
    }
    ...
    public function execute($id, $data, $finalreport) {
        ...
        // Dataset definition.
        $dataset = new core\chart_series('label', $series[1]);
        $labels = $series[0];
        $chart->add_series($dataset);
